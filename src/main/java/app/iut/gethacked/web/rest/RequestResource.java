...
public class RequestResource {
     ...
     */
    @GetMapping("/requests")
    public List<Request> getAllRequests(@RequestParam(required = false) String filter) {
        if ("contract-is-null".equals(filter)) {
            log.debug("REST request to get all Requests where contract is null");
            return StreamSupport
                .stream(requestRepository.findAll().spliterator(), false)
                .filter(request -> request.getContract() == null)
